...
from trope_clustering.models import Movie, ProcessingStatus, Characters
from trope_clustering.database import DatabaseHandler
from trope_clustering.utils import (
...
class ChatProcessor:
    ...
    def process_movie(self, movie: Movie) -> bool:
        ...
        try:
            ...
            self.db.add_trope_clustering(movie.id, result.characters)
            return True
            ...
            logging.error(f"No Internet - stopping processing")
            return False    
        except ValidationError as e:
            logging.error(f"Can not validate trope(s) for movie {movie.id}:\n{e}")
            self.db.update_movie_status(movie.id, ProcessingStatus.FAILED)
            return True
        except Exception as e:
            logging.error(f"Unexpected error for movie {movie.id}:\n{e}")
            self.db.update_movie_status(movie.id, ProcessingStatus.FAILED)
            return True
